...
</main><?php if (!empty($pageHelp)): ?>
<button type="button" class="help-fab" id="helpToggle" aria-label="راهنمای این صفحه" aria-haspopup="dialog">
  <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true"><circle cx="12" cy="12" r="9.5" fill="none" stroke="currentColor" stroke-width="2"/><path d="M9.3 9.4a2.8 2.8 0 1 1 3.9 2.6c-.8.4-1.2 1-1.2 1.8v.6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="17.3" r="1.2" fill="currentColor"/></svg>
  <span>راهنما</span>
</button>
<?php endif; ?><footer>کلیه حقوق مادی و معنوی این سامانه، متعلق به سازمان رزم سوم فراجا می‌باشد.</footer><script>
(function(){
  const fa='۰۱۲۳۴۵۶۷۸۹';
  const en='0123456789';
...
(function(){
  ...
  // هر گزینه با مقدار خالی فقط «متن راهنما»ست و هرگز داخل فهرست بازشو نمی‌آید.
  const isPlaceholder = (o)=> !!o && o.value==='';
  ...
  function build(sel){
    ...
    // اگر خالی‌گذاشتن این فیلد مجاز است، به‌جای گزینه «خالی» یک دکمه × برای پاک‌کردن نشان داده می‌شود.
    const clearable = !sel.required && Array.from(sel.options).some(o=>o.value==='' && !o.disabled);
    let clearBtn=null;
    if(clearable){
      ...
      clearBtn.addEventListener('keydown',(ev)=>{ if(ev.key==='Enter'||ev.key===' ') doClear(ev); });
    }
    ...
    function paintTrigger(){
      ...
      box.classList.toggle('has-clear', !!clearBtn && !!sel.value && !sel.disabled);
    }
...
<div class="notice-modal" id="noticeModal" aria-hidden="true" role="alertdialog" aria-modal="true" aria-labelledby="noticeTitle">
  <div class="notice-backdrop" data-notice-close></div>
  <section class="notice-card">
    <div class="notice-icon" aria-hidden="true"></div>
    <h2 id="noticeTitle"></h2>
    <div class="notice-body"></div>
    <button type="button" class="btn notice-ok" data-notice-close>تایید</button>
  </section>
</div>
<script>
/* پیام‌های خطا/موفقیت به‌جای بالای صفحه، در یک پاپ‌آپ وسط صفحه نشان داده می‌شوند. */
(function(){
  var modal=document.getElementById('noticeModal'); if(!modal) return;
  var alerts=[].filter.call(document.querySelectorAll('main .alert'),function(a){ return !a.closest('.pv-modal,.notice-modal') && a.textContent.trim()!==''; });
  if(!alerts.length) return;
  var isError=alerts.some(function(a){ return a.classList.contains('danger')||a.classList.contains('error'); });
  modal.classList.add(isError?'is-error':'is-success');
  modal.querySelector('.notice-icon').textContent=isError?'!':'✓';
  modal.querySelector('#noticeTitle').textContent=isError?'خطا':'انجام شد';
  var body=modal.querySelector('.notice-body');
  alerts.forEach(function(a){
    var p=document.createElement('p');
    p.className=(a.classList.contains('danger')||a.classList.contains('error'))?'is-error':'is-success';
    p.textContent=a.textContent.trim();
    body.appendChild(p);
    a.remove();
  });
  function close(){ modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); clearTimeout(timer); }
  modal.querySelectorAll('[data-notice-close]').forEach(function(el){ el.addEventListener('click',close); });
  document.addEventListener('keydown',function(e){ if(e.key==='Escape') close(); });
  modal.classList.add('open'); modal.setAttribute('aria-hidden','false');
  setTimeout(function(){ modal.querySelector('.notice-ok').focus(); },30);
  // پاپ‌آپ تا وقتی کاربر «تایید» را نزند بسته نمی‌شود.
  var timer=0;
  // حذف پارامترهای پیام از نوار آدرس تا با رفرش دوباره نیاید.
  try{
    var u=new URL(location.href), keys=['saved','error','success','dismissed','disc_saved','disc_auto','auto_dismissed','disc_error','dismiss_error'], changed=false;
    keys.forEach(function(k){ if(u.searchParams.has(k)){ u.searchParams.delete(k); changed=true; } });
    if(changed) history.replaceState(null,'',u.pathname+(u.searchParams.toString()?'?'+u.searchParams:'')+u.hash);
  }catch(e){}
})();
</script>
<script>
...
<script>
/* حالت روشن/تاریک و پنجره راهنما */
(function(){
  ...
  var modal=document.getElementById('helpModal'), open=document.getElementById('helpToggle');
  if(modal && open){
    if(modal.parentElement!==document.body) document.body.appendChild(modal);
    function show(){ modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); setTimeout(function(){ var c=modal.querySelector('.help-close'); if(c) c.focus(); },30); }
    function hide(){ modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); open.focus(); }
    open.addEventListener('click',show);
    modal.querySelectorAll('[data-help-close]').forEach(function(el){ el.addEventListener('click',hide); });
