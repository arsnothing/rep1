...
// ثبت آموزش فقط برای متقاضیانِ همان دوره (تب «متقاضیان آموزش»)
$hasApplicants = ensure_training_applicants_table();
if ($hasApplicants) {
    $ap = $pdo->prepare('SELECT 1 FROM training_applicants WHERE course_key=? AND personnel_id=? LIMIT 1');
    $ap->execute([$courseKey, $id]);
    if (!$ap->fetchColumn()) $back('not_applicant');
}
...
try {
    ...
    // دوره گذرانده شد؛ دیگر در فهرست متقاضیان این دوره نمی‌ماند.
    if ($hasApplicants) {
        $pdo->prepare('DELETE FROM training_applicants WHERE course_key=? AND personnel_id=?')->execute([$courseKey, $id]);
    }
