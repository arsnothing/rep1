...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!$error&&count($selectedIds)===0) $error='حداقل یک عنصر را انتخاب کنید.';
    $items=[];
    if(!$error){
        ...
        if(count($map)!==count($selectedIds)) $error='یکی از عناصر انتخاب‌ شده پیدا نشد.';
        foreach($selectedIds as $pid){
            ...
            if(!can_view_unit($map[$pid]['unit'])){$error='یکی از عناصر انتخاب‌ شده خارج از محدوده دسترسی شماست.';break;}
...
<div class="section-title wide">انتخاب عنصر / عناصر </div>
...
<div id="groupBox" class="wide" style="display:none"><label>عناصر<select name="personnel_ids[]" id="personnel_ids" multiple size="8"><?php foreach($people as $p):?><option value="<?=$p['id']?>"><?=e($p['full_name'])?> — <?=e(unit_label($p['unit']))?> — <?=e($p['national_id'])?></option><?php endforeach;?></select><small>برای واریز گروهی چند عنصر را انتخاب کنید.</small></label></div>
