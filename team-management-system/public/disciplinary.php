...
$typeLabels = ['encouragement'=>'تشویق','warning'=>'تذکر','reprimand'=>'توبیخ'];
$typeTotals = [];
...
<div class="dismiss-modal" data-dismiss-modal aria-hidden="true">
  ...
  <section class="dismiss-modal-card" role="dialog" aria-modal="true" aria-labelledby="dismissTitle">
    ...
    <form method="post" action="personnel_dismiss.php" class="dismiss-form">
      ...
      <label class="dismiss-reason-label"><span>نوع برکناری</span><select name="dismissal_type" required><option value="" selected disabled hidden>انتخاب نوع برکناری</option><option value="deputy">معاونت</option><option value="unit_commander">فرمانده دسته</option><option value="group_commander">فرمانده گروه</option><option value="resignation">استعفا</option></select></label>
