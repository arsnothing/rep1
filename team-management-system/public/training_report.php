...
$statusOptions=['trained'=>'عناصر آموزش دیده','untrained'=>'عناصر نیازمند آموزش','dismissed'=>'عناصر راکد'];
$unitOptions=['information'=>'رسته اطلاعاتی','operations'=>'رسته عملیاتی'];
...
// وضعیت عناصر و رسته، مثل تب متقاضیان آموزش، تک‌انتخابی هستند.
$statusFilter=(string)($_GET['status']??''); if(!isset($statusOptions[$statusFilter])) $statusFilter='';
$unitFilter=(string)($_GET['filter']??''); if(!isset($unitOptions[$unitFilter]) || !can_view_unit($unitFilter)) $unitFilter='';
$statuses = $statusFilter!=='' ? [$statusFilter] : [];
$units    = $unitFilter!=='' ? [$unitFilter] : [];
...
if($hasCourse){
    ...
    // عناصر راکد فقط در وضعیت «عناصر راکد» دیده می‌شوند.
    ...
    $having='';
    $onlyTrained   = ($statusFilter==='trained');
    $onlyUntrained = ($statusFilter==='untrained');
...
$courseLabel = !$courses ? 'عنوان آموزشی'
...
<div class="training-tabs training-tabs-equal">
  <?= training_tabs('training_report.php') ?>
...
<form class="training-filters" method="get" id="reportFilters" data-auto-filter>
  ...
  <div class="filter-bar training-report-filters">
    ...
    <?php endif; ?>
    <input type="text" name="category_number" id="categoryNumberInput" class="filter-control filter-number" inputmode="numeric" maxlength="3" autocomplete="off" placeholder="شماره دسته" value="<?= e($categoryNumber) ?>" aria-label="شماره دسته">
    <div class="smart-filter tf-unit" data-placeholder="انتخاب رسته">
      ...
      <select name="filter" class="smart-filter-native" aria-label="انتخاب رسته"><option value="" hidden <?= $unitFilter===''?'selected':'' ?>>انتخاب رسته</option><?php foreach($unitOptions as $k=>$v):?><option value="<?=e($k)?>" <?= $unitFilter===$k?'selected':'' ?>><?=e($v)?></option><?php endforeach;?></select>
    </div>
    <div class="smart-filter tf-status" data-placeholder="وضعیت عناصر">
      ...
      <div class="smart-filter-menu"><div class="smart-filter-options" role="listbox"></div></div>
      <select name="status" class="smart-filter-native" aria-label="وضعیت عناصر"><option value="" hidden <?= $statusFilter===''?'selected':'' ?>>وضعیت عناصر</option><?php foreach($statusOptions as $k=>$v):?><option value="<?=e($k)?>" <?= $statusFilter===$k?'selected':'' ?>><?=e($v)?></option><?php endforeach;?></select>
    </div>
...
<?php if(!$hasCourse): ?>
  <div class="personnel-empty"><strong>گزارشی نمایش داده نشده است.</strong></div>
