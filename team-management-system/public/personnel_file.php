...
if($type==='photo'){
    $st=$pdo->prepare('SELECT p.id,ct.category_key AS unit,p.profile_photo_path FROM personnel p JOIN category_types ct ON ct.id=p.category_type_id WHERE p.id=?');$st->execute([$id]);$r=$st->fetch();
    if(!$r||!can_view_unit($r['unit'])){http_response_code(404);exit('فایل یافت نشد.');}
    $path=profile_photo_exists($r['profile_photo_path']) ? __DIR__.'/../storage/profile_photos/'.basename((string)$r['profile_photo_path']) : '';
    $mime=($path!=='' && is_file($path)) ? (mime_content_type($path)?:'') : '';
    // عکس ندارد یا فایلش خراب/ناموجود است: آیکن پیش‌فرض پروفایل (آدمک)
    if($mime==='' || !str_starts_with($mime,'image/')){
        $path=__DIR__.'/assets/default-avatar.svg'; $mime='image/svg+xml';
    }
}
