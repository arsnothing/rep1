...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if(($_POST['action']??'')==='add_order'){
  try{
   ...
   $equipmentIds=array_values(array_unique(array_filter(array_map('intval',(array)($_POST['equipment_ids']??[])))));
   $pickedEquipment=[];
   $expiryDate=null;
   ...
   if($type==='mission'){
    ...
    // تجهیزات ماموریت: فقط از آمادهای سالم/تعمیرشده و آزاد؛ سلاح فقط در ماموریت «با سلاح»
    if($equipmentIds){
     $available=[]; foreach(available_mission_equipment() as $it) $available[(int)$it['id']]=$it;
     foreach($equipmentIds as $eid){
      if(!isset($available[$eid])) throw new RuntimeException('یکی از تجهیزات انتخاب‌شده دیگر در دسترس نیست (خراب، در تعمیر یا در ماموریت دیگر). صفحه را دوباره باز کنید.');
      if(equipment_is_weapon($available[$eid]['equipment_type']) && $weaponStatus!=='with') throw new RuntimeException('برای ماموریت بدون سلاح نمی‌توان سلاح انتخاب کرد.');
      $pickedEquipment[]=$available[$eid];
     }
     if(!ensure_order_equipment_table()) throw new RuntimeException('ذخیره تجهیزات حکم ممکن نیست. فایل database/team_management.sql را اجرا کنید.');
    }
    // اگر سلاح از فهرست آماد انتخاب شده، نوع و سریال آن خودکار پر می‌شود.
    if($weaponStatus==='with'){
     foreach($pickedEquipment as $it){
      if(!equipment_is_weapon($it['equipment_type'])) continue;
      if($weaponType==='') $weaponType=trim((string)($it['model']?:$it['equipment_type']));
      if($weaponSerial==='') $weaponSerial=trim((string)$it['serial_number']);
      break;
     }
    }
    ...
    $pickedEquipment=[]; $durationDays=0; $orderNumber=''; $weaponStatus=''; $weaponType=''; $weaponSerial=''; $purpose=''; $vehicleType=''; $destination='';
   }
   ...
   if($type==='mission' && !$hasMissionCols) throw new RuntimeException('ساختار دیتابیس نیاز به به‌روزرسانی دارد. فایل database/team_management.sql را اجرا کنید.');
   ...
   if($type==='mission' && $pickedEquipment){
    $eqIns=$pdo->prepare('INSERT INTO personnel_order_equipment(order_id,stock_id,equipment_type,serial_number,plate,model,created_by) VALUES(?,?,?,?,?,?,?)');
    foreach($pickedEquipment as $it) $eqIns->execute([$orderId,(int)$it['id'],$it['equipment_type'],$it['serial_number'],$it['plate'],$it['model'],user()['id']]);
   }
   $pdo->commit(); $success='حکم با موفقیت ثبت شد.'.($pickedEquipment?' ('.fa_digits((string)count($pickedEquipment)).' قلم تجهیزات برای ماموریت ثبت شد.)':'');
...
$missionEquipment = $activeTab==='add' ? available_mission_equipment() : [];
$orderEquipmentMap=[];
if($activeTab==='list' && ensure_order_equipment_table()){
 foreach($pdo->query('SELECT order_id,equipment_type,serial_number,plate,model FROM personnel_order_equipment ORDER BY order_id,equipment_type,id')->fetchAll() as $eqr){
  $orderEquipmentMap[(int)$eqr['order_id']][]=trim($eqr['equipment_type'].($eqr['model']?' '.$eqr['model']:'').(($eqr['plate']?:$eqr['serial_number'])?' — '.fa_digits((string)($eqr['plate']?:$eqr['serial_number'])):''));
 }
}
...
<section class="page-head"><div><h1>احکام</h1></div></section>
...
<div class="panel order-form-panel">
 <form method="post" class="form-grid" id="orderForm" enctype="multipart/form-data">
  ...
  <div class="order-person-picker wide">
    <div class="order-person-head"><div><strong id="orderPersonTitle">انتخاب عنصر</strong></div><span id="orderPersonState">انتخاب نشده</span></div>
  ...
  <div class="mission-only wide order-person-picker mission-equip-picker" id="equipmentPicker">
    <div class="order-person-head"><div><strong>تجهیزات ماموریت</strong></div><span id="selectedEquipCount">۰ قلم انتخاب شده</span></div>
    <?php if($missionEquipment): ?>
    <div class="order-person-search">
      <input type="search" id="equipSearch" class="filter-control filter-search" placeholder="جستجو نوع، مدل، سریال یا پلاک" autocomplete="off">
    </div>
    <div class="order-person-list" id="equipList">
      <?php foreach($missionEquipment as $it): $isWeapon=equipment_is_weapon($it['equipment_type']); $unitNo=$it['plate']?:$it['serial_number']; ?>
      <label class="op-item equip-row" data-weapon="<?= $isWeapon?'1':'0' ?>" data-search="<?=e(mb_strtolower($it['equipment_type'].' '.$it['model'].' '.$unitNo.' '.fa_digits((string)$unitNo),'UTF-8'))?>" data-type="<?=e($isWeapon?($it['model']?:$it['equipment_type']):'')?>" data-serial="<?=e($isWeapon?(string)$it['serial_number']:'')?>"<?= $isWeapon?' hidden':'' ?>>
        <input type="checkbox" name="equipment_ids[]" value="<?=(int)$it['id']?>"<?= $isWeapon?' disabled':'' ?>>
        <span class="op-name"><?=e($it['equipment_type'])?><?php if($it['model']):?> <small><?=e($it['model'])?></small><?php endif;?></span>
        <span class="op-nid"><?= $unitNo ? ($it['plate']?'پلاک ':'').e(fa_digits((string)$unitNo)) : '—' ?></span>
        <span class="status-chip <?=e(equipment_status_chip_class($it['status']))?>"><?=e(equipment_status_options()[$it['status']]??'')?></span>
      </label>
      <?php endforeach; ?>
    </div>
    <div class="order-person-empty" id="equipEmpty" hidden>موردی پیدا نشد.</div>
    <?php else: ?>
    <div class="order-person-empty">در انبار آماد، تجهیز سالم یا تعمیرشده‌ای برای واگذاری وجود ندارد.</div>
    <?php endif; ?>
  </div>

  <div class="mission-only wide order-person-picker" id="subordinatePicker">
    <div class="order-person-head"><div><strong>همراهان حکم</strong></div><span id="selectedMemberCount">۰ عنصر انتخاب شده</span></div>
    <div class="order-person-search">
...
<div class="panel print-list orders-list-panel">
 ...
 <div class="table-wrap"><table><thead><tr><th>ردیف</th><th>شماره حکم</th><th>نام و نام خانوادگی</th><th>کد ملی</th><th>نوع حکم</th><th>سمت / به منظور</th><th>صدور</th><th>انقضا</th><th>تمدید</th><th>همراهان حکم</th><th>تجهیزات</th><th>توضیحات</th></tr></thead><tbody><?php $orderRow=0; foreach($orders as $o): $orderRow++; $members=$orderMemberMap[(int)$o['id']]??[]; ?><tr><td><?=fa_digits((string)$orderRow)?></td><td><?=e(fa_digits((string)($o['order_number']??''))?:'—')?></td><td><a href="personnel_view.php?id=<?=(int)$o['personnel_id']?>&tab=orders"><strong><?=e($o['full_name'])?></strong></a></td><td><?=e($o['national_id']?:'—')?></td><td><?=$o['order_type']==='mission'?'ماموریتی':'مسئولیتی'?></td><td><?=e($o['responsibility_position']?:($o['title']?:'—'))?></td><td><?=jalali_display($o['order_date'])?></td><td><?=$o['order_type']==='mission'?jalali_display($o['end_date']):'—'?></td><td><?= !empty($o['renewed_until']) ? jalali_display($o['renewed_until']) : '—' ?></td><td class="cell-members"><?php if($members):?><button type="button" class="members-btn" data-pv-open="membersModal" data-order-number="<?=e(fa_digits((string)($o['order_number']??''))?:'—')?>" data-members="<?=e(json_encode(array_column($members,'full_name'),JSON_UNESCAPED_UNICODE))?>"><?=fa_digits((string)count($members))?> عنصر</button><?php else:?>—<?php endif;?></td><td class="cell-members"><?php $eqs=$orderEquipmentMap[(int)$o['id']]??[]; if($eqs):?><button type="button" class="members-btn" data-pv-open="equipModal" data-order-number="<?=e(fa_digits((string)($o['order_number']??''))?:'—')?>" data-equipment="<?=e(json_encode($eqs,JSON_UNESCAPED_UNICODE))?>"><?=fa_digits((string)count($eqs))?> قلم</button><?php else:?>—<?php endif;?></td><td><?=e($o['description']?:'—')?></td></tr><?php endforeach;if(!$orders):?><tr><td colspan="12" class="empty">حکمی با این جستجو پیدا نشد.</td></tr><?php endif;?></tbody></table></div>
</div>
<?php endif; ?>
<script>
(function(){
 ...
 /* ---------- تجهیزات ماموریت (سلاح فقط با «با سلاح») ---------- */
 const equipSearch=document.getElementById('equipSearch');
 const equipEmpty=document.getElementById('equipEmpty');
 const equipCount=document.getElementById('selectedEquipCount');
 const equipRows=[...document.querySelectorAll('.equip-row')];
 let autoWeapon=false;
 function updateEquipment(){
  const withWeapon=!!wsEl && wsEl.value==='with' && typeEl.value==='mission';
  const mission=typeEl.value==='mission';
  const q=faFold((equipSearch?.value||'').trim());
  let shown=0, selected=0, weaponPick=null;
  equipRows.forEach(r=>{
   const cb=r.querySelector('input');
   const allowed=r.dataset.weapon!=='1' || withWeapon;
   if(!allowed) cb.checked=false;
   cb.disabled=!allowed || !mission;
   const hit=allowed && (!q || faFold(r.dataset.search||'').includes(q));
   r.hidden=!hit && !(allowed && cb.checked);
   if(!r.hidden) shown++;
   if(cb.checked){ selected++; if(r.dataset.weapon==='1' && !weaponPick) weaponPick=r; }
  });
  if(equipEmpty) equipEmpty.hidden=shown!==0 || !equipRows.length;
  if(equipCount) equipCount.textContent=new Intl.NumberFormat('fa-IR').format(selected)+' قلم انتخاب شده';
  // نوع و سریال سلاح از سلاحِ انتخاب‌شده پر می‌شود.
  if(withWeapon && wtypeEl && serialEl){
   if(weaponPick){
    wtypeEl.value=weaponPick.dataset.type||''; serialEl.value=toFa(weaponPick.dataset.serial||'');
    wtypeEl.readOnly=serialEl.readOnly=true; autoWeapon=true;
   }else if(autoWeapon){
    wtypeEl.value=''; serialEl.value=''; wtypeEl.readOnly=serialEl.readOnly=false; autoWeapon=false;
   }
  }else if(wtypeEl && serialEl){ wtypeEl.readOnly=serialEl.readOnly=false; autoWeapon=false; }
 }
 equipSearch?.addEventListener('input',()=>updateEquipment());
 equipSearch?.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); updateEquipment(); } });
 equipRows.forEach(r=>r.querySelector('input').addEventListener('change',()=>updateEquipment()));
 ...
 function updateMembers(){
  ...
  if(memberCount) memberCount.textContent=new Intl.NumberFormat('fa-IR').format(selected)+' عنصر انتخاب شده';
 }
...
<div class="pv-modal" id="equipModal" aria-hidden="true">
  <div class="pv-modal-backdrop" data-pv-close></div>
  <section class="pv-modal-card" role="dialog" aria-modal="true" aria-labelledby="equipModalTitle">
    <header class="pv-modal-head">
      <div><span class="pv-modal-eyebrow">حکم شماره <span data-order-number-target>—</span></span><h2 id="equipModalTitle">تجهیزات ماموریت</h2></div>
      <button type="button" class="pv-modal-close" data-pv-close aria-label="بستن">×</button>
    </header>
    <div class="members-viewer" data-equipment-list></div>
    <div class="pv-modal-actions"><button type="button" class="btn secondary" data-pv-close>بستن</button></div>
  </section>
</div>
<?php endif; ?>
...
<script>
/* پاپ‌آپ اعضای زیرحکم */
document.querySelectorAll('[data-equipment]').forEach(function(btn){
 btn.addEventListener('click',function(){
  var box=document.querySelector('[data-equipment-list]'); if(!box) return;
  var items=[]; try{ items=JSON.parse(btn.dataset.equipment||'[]'); }catch(e){ items=[]; }
  box.innerHTML = items.length
   ? '<ol class="members-list">'+items.map(function(n){ return '<li>'+String(n).replace(/[<>&]/g,'')+'</li>'; }).join('')+'</ol>'
   : '<div class="cert-empty">تجهیزی ثبت نشده است.</div>';
 });
});
